...
+device driver는 소프트웨어
device driver는 CPU가 device controller에게 부탁을 하는 기계어(예를 들면, 필요한 디스크 파일을 가져와달라고할때) 역할을 함.(device driver는 CPU가 실행하는 코드이다. device controller가 실행하는 코드 아님!)
...
-메모리를 접근하는 기계어 따로, I/O접근하는 기계어를 따로두는 방법 -메모리를 접근하는 기계어로 I/O도 접근하는 방법
