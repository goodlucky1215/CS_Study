...
[Context Switch 문맥 교환] 2강의 10분전부터 다시보기
CPU에서 프로세스A에서 프로세스B로 넘긴다고할때, ProcessA가 어디까지 실행했는지 알기위한 정보들을 PCB에 저장해둔다. Program Counter와 Registers를 PCB에서 담아두고 실행됐을 때 이 정보로 문맥을 읽어온다.
...
- exec는 execlp를 실행시 완전히 새로운 프로그램이 됨. 그래서 그 밑에 printf("sksk");라고 찍어서 실행해도 sksk가 실행되지 않는다.
- wait()는 fork()로 자식을 생성했을 때, 자식이 종료될 때까지, 부모를 wait()로 걸어서 움직이지 못하게 하는 상태(blocked)로 만드는 것이다.
  Q. wait를 왜 사용할까?
...
- exit() 프로세스를 종료 시키는 것임, 넣지 않아도 main함수가 리턴되는 위치에 알아서 컴파일러가 넣어줌
...
- 협력 매커니즘(IPC: Interprocess Communication)
  방법2가지
