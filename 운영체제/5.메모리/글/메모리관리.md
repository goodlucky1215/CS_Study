...
=> 주소 바인딩 된 물리적 메모리 기계어들의 안에 만들어진 것아 논리적인 주소이기 때문이다. 실행파일의 0번지 주소의 Add 20, 30 에서의 20과 30은 주소번지. 이게 Physical address변환 시에도 안의 Add 20, 30은 변하지 않는 것을 알 수 있다. 그렇기 때문에 CPU가 그때그때 logical address를 Physical address로 주소 변환을 해서 사용을 한다. => 특히나 Run time binding은 실행 도중에도 Physical address 변경되어서 매번 주소를 참조할 때마다 바인딩을 점검해야한다.
...
- 그림 참고
  virtual memory가 메모리에 올라갈 때를 나타낸 그림.(연속 할당)
...
- Overlays : 메모리에 프로세스의 필요한 부분만 올림. 초창기에 메모리 공간이 작아서, 프로그래머가 구현해서 메모리에 올리고 내림.(프로그램이 매우 복잡)
- Swapping
...
[Physical Memory(물리적인 메모리)] -물리적인 메모리 : OS영역 + 사용자 프로세스 영역
...
First-fit : hole들 중에 최초로 찾아지는 곳에 프로세스 할당
Best-fit : hole들 중에서 프로세스를 넣고 내부조각이 가장 적게 남을 수 있는 공간 Worst-fit : hole들 중에서 가장 공간이 큰 곳에 프로세스를 할당.
compaction : hole들을 모아서 메모리 공간을 넓히는 것 => 모든 메모리가 다 이동해야 할 수 있어 매우 복잠함. Run time binding만 가능하겠지!
...
a. Paging : 프로세스의 virtual memory를 동일한 사이즈의 page단위로 나눔
...
TLB는 캐시메모리여서 당연히 메모리에 접근하여 page table에서 찾는 거보다 빠르다. \*캐시메모리는 데이터 접근을 빨리 하기위한 용도, 주소 변환을 빨리 하기위한 용도 2가지가 있다.

- Tow-Level Page table
  logical memory의 page가 백만개이면 page table도 백만개의 값을 가진다. physical memory에 올라가는 page는 극히 일부이다. main memory에는 그러면 사용하지도 않는 page들의 page table도 전부 올라가 있다는 것 => 그래서 Tow-Level Page table을 사용.
...
b. Protection bit
해당 page에 대한 접근 권한(read/write/read-only) ex) page 내용중에 code부분은 이미 작성 되어 있는 것이고 이건 read만 해야지 바꾸면 안되므로 read-only여만 한다. => page는 동일한 크기로 잘라서 여러 의미를 가진 부분(code와 stack의 일부라던가..)끼리 같이 합쳐져서 나올 수 있다. 그렇게 되면 read/write/read-only 무엇을 해야할지 애매해지는 문제가 있다.
...
STBR(Segment-table base register) 물리적 메모리에서의 page table을 위치
STLR(Segment-table length register)프로그램이 사용하는 Segment의 수

ex) limit이 segment의 크기, base가 시작 위치 임을 볼 수 있다.
장점 :  
 Protection(code는 read-only만 해야하고 stack은 read와 write가 다 가능해야하는 등 이러한 접근 권한을 부여하기가 쉽다)
...
기본적으로 Segmentation기법을 사용하는데, segment가 각각 여러개의 page로 구성되는 것이다. ex) 1번 segment는 page 3개로 구성 되고, 2번 segment는 page 5개로 구성 되고 등등
